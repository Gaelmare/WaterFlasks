...
def make_book(rm: ResourceManager, i18n: I18n, local_instance: bool = False, reverse_translate: bool = False):
    ...
    book.category('waterflasks', 'Water Flasks', 'Have some better containers for carrying your drinks around with you.', 'waterflasks:iron_flask', is_sorted=True, entries=(
        entry('bladders', 'Flask Materials', 'waterflasks:bladder', pages=(
            ...
            text('$(br)$(bold){:_<12s}'.format('Animal') + '{:_>17s}'.format('Chance$(br)') +'$()'+''.join([('{0:_<16s}{1:_>10s}%').format(animal, DROP_ENTITIES[animal])+'$(br)' for animal in DROP_ENTITIES]) + '$(br)To increase your chances, use a higher damage weapon to butcher the animal.'),
            item_spotlight('waterflasks:leather_side', 'Leather Flask Side', text_contents='Flask Sides are used in flask recipes.').link('waterflasks:leather_side'),
        ...
        entry('recipes', 'Flask Recipes', 'waterflasks:leather_flask', pages=(
            text('There are three tiers of water flasks.$(br)The leather flask holds 5 drinks and lasts for 100 uses.$(br)The iron flask holds 20 drinks, and lasts for 400 uses.$(br)The red steel flask holds 20 drinks, and lasts forever.$(br)Both leather and iron flasks may be repaired.'),
